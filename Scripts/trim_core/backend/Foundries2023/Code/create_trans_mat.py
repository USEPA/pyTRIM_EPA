...
def link_check (comp_objects_dict,comp1_name,comp2_name,dict_inputs,chem_list_clean,currentchemical): # function to check if two compartments are potentially linked and to return connecting algorithms
    ...
    if cond1 or cond2:    ### THIS NEEDS A MORE ROBUST FILTERING APPROACH. TOO MANY IFS.
        ...
        if 'terrestrial plant | stem | stem - grasses/herbs' in comp1.category and 'terrestrial plant | stem | stem - grasses/herbs' in comp2.category: # to cover cases where comp1 category is 'terrestrial plant | stem | stem - grasses/herbs' and comp2 category is 'terrestrial plant | stem | stem - grasses/herbs'
            ...
            df_app=df_app.append(df_app2) 
        if 'fish' in comp1.category and 'fish' in comp2.category: # to cover cases where comp1 category = fish | something and comp2 category = fish | something 
            df_app2=df_alg_mat.loc[(df_alg_mat['sendingcompartmentcategory']=="fish") & (df_alg_mat['receivingcompartmentcategory']=="fish") & (df_alg_mat['enabled']=='True')]
            df_app=df_app.append(df_app2)
...
def create_trans_mat(inputs,dict_inputs): # function to create transition matrix
    ...
    c0=np.zeros(mat_dim,dtype=float) # create zero initial concentrations matrix
    c0_units=[''] * mat_dim # create empty list of c0 units
    ...
    for chem_index, chem in enumerate(chem_list_clean): # loop over chemicals
        ...
        for row_index, comp_row in enumerate(comp_list): # loop over rows (sending compartments)
            ...
            if hasattr(comp_objects_dict[comp_row], 'initialconcentration_g_per_m3'): # if sending compartment has deposition rate
                if currentchemical.name in comp_objects_dict[comp_row].initialconcentration_g_per_m3.keys(): # if there is deposition of the current chemical:
                    c0[int(chem_index*ncomp+row_index)]=c0[int(chem_index*ncomp+row_index)]+comp_objects_dict[comp_row].initialconcentration_g_per_m3[currentchemical.name] # add depostion rate
                    c0_units[row_index]='g_per_m3'
            if hasattr(comp_objects_dict[comp_row], 'initialconcentration_g_per_kg'): # if sending compartment has deposition rate
                if currentchemical.name in comp_objects_dict[comp_row].initialconcentration_g_per_kg.keys(): # if there is deposition of the current chemical:
                    c0[int(chem_index*ncomp+row_index)]=c0[int(chem_index*ncomp+row_index)]+comp_objects_dict[comp_row].initialconcentration_g_per_kg[currentchemical.name] # add depostion rate
                    c0_units[row_index]='g_per_kg'
            if hasattr(comp_objects_dict[comp_row], 'initialconcentration_g_per_l'): # if sending compartment has deposition rate
                if currentchemical.name in comp_objects_dict[comp_row].initialconcentration_g_per_l.keys(): # if there is deposition of the current chemical:
                    c0[int(chem_index*ncomp+row_index)]=c0[int(chem_index*ncomp+row_index)]+comp_objects_dict[comp_row].initialconcentration_g_per_l[currentchemical.name] # add depostion rate
                    c0_units[row_index]='g_per_l'
    ...
    df_c0=pd.DataFrame({'initial_concentration':c0,'initial_concentration_units':c0_units,},index=ind_name)
    df_n0=compute_initial_mass(df_c0,df_vmu)     # dataframe of initial masses computed      
    return(tm,sm,vmu,df_tm,df_sm,df_vmu,df_n0)
...
# row_index=95 # elemental wch sw_cadillac
# col_index=95 # divalent wch cadillac
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 
...
# row_index=28 # elemental surf soil e1
# col_index=0 # air in e2
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 

# row_index=216 # elemental dry vapor ps in e1
# col_index=110 # grasses hervbts in e1
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 

# row_index=22 # sw mitchell
# col_index=101 # flus sink in mitchell
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 

# row_index=24 # surf soil e1
# col_index=0 # air e1
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
...
# row_index=216 # chem_elemental_mercury_dryvaporsource_in_dryvaporsource_e1
# col_index=24# surfsoil_e1
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 
