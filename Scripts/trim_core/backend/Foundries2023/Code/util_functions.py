...
def process_met(inputs): ## one time process all met weighted averages
    ...
    simulation_start_date=pd.to_datetime(inputs['simulation_start_date'])
    simulation_end_date=pd.to_datetime(inputs['simulation_end_date'])  
    ...
    df['DT']=list(pd.to_datetime(df[['Year', 'Month', 'Day','Hour']],errors='coerce')) # create datetime timestamp objects
    df=df[(df['DT']<simulation_end_date)&(df['DT']>=simulation_start_date)]# keep only observations within simulation time bounds
    
    df['date_delta'] = (df['DT'] - df['DT'].min())  / timedelta64(1,'D') # time series in units of day with zero set at lowest datetime point
    df['time_delta']=df['date_delta'].diff() # time elapsed difference between successive rows in units of day
    ...
    df['DT_Check']=df.DT>=(df.DT.shift()) # flag for sequential observation
    df=df[df['DT_Check']] # drop non sequential
   ...
   ### need to clean up messy met file to get reasonable averages. This shouldnt be required with a quality met file.             
    ...
    df2=df2[(df2['DT']<simulation_end_date)&(df2['DT']>=simulation_start_date)]# keep only observations within simulation time bounds
