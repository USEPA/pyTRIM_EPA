...
''')   
        ...
        if type(name)==str:
            ...
            if r'c\\' in cname:
                try:
                    ...
                    met_key_name="inputs['met_dict']['vector_"+metcol+"']"
    ...
    def add_quotes(name): # function to add quotes to strings containing \ or / ## this is very hacky for now
        ...
        if type(name)==str and (r'\\' in name or '_' in name or name=='annual') and ('met_dict' not in name):
            cname="'"+name+"'"
            return (cname)
        return(name)
...
#### Read, process, and write other properties file
    ...
    return(df_props,df_props_2,df_links)