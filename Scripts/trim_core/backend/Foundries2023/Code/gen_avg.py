...
def gen_avg(df_nt,df_conc,inputs):
    ...
    # following block is a convoluted way of obtaining a dataframe with the same columns as dfc_avg (created below) but with a single row containing units (will later concat dfc_avg to this)
    conc_cols=list(df_conc.columns) # columns in df_conc
    unit_cols=[x for x in conc_cols if '_unit' in x] # cols containing units (order preserved)
    dfc_avg_units=df_conc.head(1)[unit_cols] # Dataframe with single row containing units only
    dfc_avg_units['time']='' # add blank time col
    dfc_avg_units['year']=''     # add blank year col
    dfc_avg_units=dfc_avg_units[['time']+unit_cols+['year']] # reorder same as dfc_avg
    dfc_avg_units.columns=[x for x in conc_cols if '_unit' not in x] # rename columns same as dfc_avg 
    reqcols=[x for x in list(dfc_avg_units.columns) if x not in ['time','year']]
    dfc_avg_units=dfc_avg_units[['year']+reqcols]
    ...
    dfc_avg=pd.concat([dfc_avg_units,dfc_avg],ignore_index=True)
