...
#        prop=prop.replace('sendingcompartment','self.sendingcompartment')
        ...
        if 'receivingvolumeelementsumof[terrestrial plant | leaf]'in prop:
            prop=prop.replace('receivingvolumeelementsumof[terrestrial plant | leaf].','self.receivingcompartment.associated_leaf_comp.')
        

        if 'receivingvolumeelementsumof[abiotic | soil | surface soil]'in prop:
            prop=prop.replace('receivingvolumeelementsumof[abiotic | soil | surface soil].','self.receivingcompartment.associated_soil_comp.')
        ...
        if 'self.sendingcompartment.allowexchange_forother' in prop and 'self.dict_inputs["met_dict"]["frac_time_rain"]' in prop: # to address interaction between exchange and rain
            ...
            prop=prop.replace('self.dict_inputs["met_dict"]["frac_time_rain"]','(self.dict_inputs["met_dict"]["vector_allowexchange"] if ("coniferous" not in self.sendingcompartment.name) else 1)')
        ...
        if ('self.containingscenario.rain') in prop:
            prop=prop.replace('self.containingscenario.rain','self.dict_inputs["met_dict"]["vector_rain"]')

        if ('self.containingscenario.airtemperature_k') in prop:
            prop=prop.replace('self.containingscenario.airtemperature_k','self.dict_inputs["met_dict"]["vector_airtemperature"]')

        if ('self.containingscenario.horizontalwindspeed') in prop:
            prop=prop.replace('self.containingscenario.airtemperature_k','self.dict_inputs["met_dict"]["vector_horizontalwindspeed"]')


        return (prop)
    

    def alternative_tf(tf): # to fix issue with associated leaf compartment on vegetationless compartments, return two alternative tfs, with the second scrubbed to remove references to the associated leaf compartment
        if 'receivingcompartment.associated_leaf_comp.' in tf:
            tf=tf.replace(')',' )')
            pattern='self.receivingcompartment.associated_leaf_comp.\S+'                    
            tf2=re.sub(pattern,'0',tf)
        elif 'sendingcompartment.associated_leaf_comp.' in tf:
            tf=tf.replace(')',' )')
            pattern='self.sendingcompartment.associated_leaf_comp.\S+'                    
            tf2=re.sub(pattern,'0',tf)
        else:
            tf2=tf            
        return(tf2)    
    ...
    with open(ofpn, 'w') as f:  
        ...
        for index,group in enumerate(grouped_alg.groups):
            ...
            tf=clean_props(tf) 
            tf2=alternative_tf(tf) # version of tf scrubbed of references to associated_leaf_compartment if any
            if tf!=tf2:
                print ('XXXXXXXXXXXX')
                print (alg_name,tf2)
...
### testing method decorator approach
            ...
            f.write('\n\t'+\
                    ...
                    'if (hasattr(self.receivingcompartment,"associated_leaf_comp") or hasattr(self.sendingcompartment,"associated_leaf_comp")):'+'\n\t\t\t'    
                    'try:'+'\n\t\t\t\t' +\
                    'r='+tf+'\n\t\t\t' +\
                    'except:'+'\n\t\t\t\t' +\
                    'r=nan'+'\n\t\t'+\
    ...
    def clean_props(prop): # function to convert properties to pythonic syntax, 
        ...
        if ('self.containingscenario.rain') in prop:
            prop=prop.replace('self.containingscenario.rain','self.inputs["met_dict"]["vector_rain"]')

        if ('self.containingscenario.airtemperature_k') in prop:
            prop=prop.replace('self.containingscenario.airtemperature_k','self.inputs["met_dict"]["vector_airtemperature"]')

        if ('self.containingscenario.horizontalwindspeed') in prop:
            prop=prop.replace('self.containingscenario.airtemperature_k','self.inputs["met_dict"]["vector_horizontalwindspeed"]')
...
## testing method decorator approach
        ...
        for index,group in enumerate(grouped_chem.groups):
            ...
            f.write('\n\n'+'class '+str(chem_name)+':'+'\n' +\
                    '\t'+\
                    'def __init__(self,constants,containingscenario,inputs):\n\t\t'+\
                    'self.containingscenario=containingscenario\n\t\t'+\
                    'self.inputs=inputs\n\t\t'+\
                    'self.constants=constants\n\t\t'+\
        ...
        for k,v in chem_dict.items():
            if v not in sim_chem_list_clean:
                continue
            f.write(str(v)+'='+str(v)+'(constants,containingscenario,inputs)'+'\n')
    ...
    def clean_props(prop): # function to convert properties to pythonic syntax, 
        ...
        prop=prop.replace('withincontainingvolumeelement[abiotic | soil | surface soil].area','self.associated_soil_comp.area')
        prop=prop.replace('self.allowexchange_dynamic','self.met_dict["vector_allowexchange"]') # Call dynamic allow exchange defined at top of script
        prop=prop.replace('self.wetvolumeperarea * self.containingvolumeelement.area if self.allowexchange_forother > 0 else 0','self.wetvolumeperarea * self.containingvolumeelement.area') ### NOT SURE. INTENT IS TO ENSURE THAT VOLUME PROPERTY OF LEAF IS NOT A TIME SERIES. DOES THIS AFFECT RESULTS?

        if ('self.containingscenario.rain') in prop:
            prop=prop.replace('self.containingscenario.rain','self.met_dict["vector_rain"]')

        if ('self.containingscenario.airtemperature_k') in prop:
            prop=prop.replace('self.containingscenario.airtemperature_k','self.met_dict["vector_airtemperature"]')

        if ('self.containingscenario.horizontalwindspeed') in prop:
            prop=prop.replace('self.containingscenario.airtemperature_k','self.met_dict["vector_horizontalwindspeed"]')

        if ('self.containingscenario.cumulativerain') in prop:
            prop=prop.replace('self.containingscenario.cumulativerain','self.met_dict["vector_cumulativerain"]')

        if ('containingscenario.isday_dynamic') in prop:
            prop=prop.replace('containingscenario.isday_dynamic','met_dict["vector_isday"]')
...
# method decorator approach 
        ...
        for index,group in enumerate(grouped_comp.groups): # loop over compartments
            ...
            f.write('\n'+'class '+str(comp_name)+':'+'\n' +\
                    '\t'+\
                    'def __init__(self,constants,containingscenario,currentchemical,containingvolumeelement,comp_objects_dict,met_dict):\n\t\t'+\
                                    'self.containingscenario=containingscenario\n\t\t'+\
                                    'self.currentchemical=currentchemical\n\t\t'+\
                                    'self.constants=constants\n\t\t'+\
            ...
            # f.write('\n\t'+\
            #     'def deposition_rate(self):'+'\n\t\t'+\
            #         'if not hasattr(self,"_deposition_rate"):\n\t\t\t'+\
            #             'self._deposition_rate'+'={}'+'\n')
...
# 	def deposition_rate(self,value):
# 		self._deposition_rate=value
...
#            comp_props=list(grouped_comp.get_group(group)['propertyname'].unique()) # unique properties
            ...
            for prop in set(all_props):
                ...
                if len(chems)==1 and type(chems[0])== float:#isnan(chems[0]): # if only nan:
                    ...
                    if 'coniferous' in group and "met_dict['vector_allowexchange']" in prop_val: # hack to deal with coniferous plants having allow exchange of 1
                        prop_val=prop_val.replace("met_dict['vector_allowexchange']",'1')

                    if 'coniferous' not in group and "met_dict['litterfallrate']" in prop: # hack to deal with non coniferous plants needing to be assigned vector_litterfallrate
                        prop_val="met_dict['vector_litterfallrate']"
