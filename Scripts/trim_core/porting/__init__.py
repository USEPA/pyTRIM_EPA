...
def load_data(trim_file_root, scenario_name, import_rules):
    ...
    elif not isinstance(import_rules, dict):
        import_rules = {}
    ...
    for param in default_entries.get('compartment_extra_params', []):
        if param[3] is None:
            par_domain = None
        else:
            par_domain = ParameterService.domains.get(name=param[3])
        Compartment.parameters.add(
            param[0], value=param[1], unit=param[2], domain=par_domain
        )

    for param in default_entries.get('compartment_overwrite_params', []):
        if param[3] is None:
            par_domain = None
        else:
            par_domain = ParameterService.domains.get(name=param[3])
