...
from define_air_algs import *
from define_attributes_props import *
from define_attributes_dep_rates import *
...
def link_check (comp_objects_dict,comp1_name,comp2_name,dict_inputs,chem_list_clean,currentchemical): # function to check if two compartments are potentially linked and to return connecting algorithms
    ...
    if cond2 or cond4: # only if cond2,3,4 do not apply, check if compartments are in neighboring vol elements. (in some cases, physically contiguous compartments wont be checked because they are already manually connected -- not sure if this is restrictive)
        cond1=False
    else:
        ...
        if cond1:
            ...
            if (comp1.category=='abiotic | soil | surface soil | surface soil - default') and (comp2.category=='abiotic | soil | root zone | root zone - default') and (comp1.parcel_name!=comp2.parcel_name): #disallow rootzone to connect to surface soil if not in same ve
                cond1=False
            if (comp1.category=='abiotic | air | air - default') and (comp2.category=='sink | abiotic | air | air - default') and (comp1.parcel_name!=comp2.parcel_name): # disallow air to air sink connections if not in same ve
                cond1=False
            if (comp1.category=='abiotic | air | air - default') and (comp2.category=='sink | abiotic | air | air - default') and ("upperair" in comp2.name): # disallow air to upper air sink connections -- temporary fix
                cond1=False
    ...
    if cond1 or cond2:    ### THIS NEEDS A MORE ROBUST FILTERING APPROACH. TOO MANY IFS.
        ...
        if 'abiotic | surface water | surface water - default' in comp1.category and 'fish' in comp2.category: # to cover cases where comp2 category = fish | something but algorithm receiving comp category is just fish
            ...
            df_app=df_app.append(df_app2)  
        if 'abiotic | air | air - default' in comp1.category and 'terrestrial plant | leaf |' in comp2.category: # to cover cases where comp1 category is air and comp2 category is terrestrial plant | leaf | something
            df_app2=df_alg_mat.loc[(df_alg_mat['sendingcompartmentcategory']=="abiotic | air | air - default") &(df_alg_mat['receivingcompartmentcategory']=="terrestrial plant | leaf") & (df_alg_mat['enabled']=='True')]
            df_app=df_app.append(df_app2) 
        if 'abiotic | air | air - default' in comp1.category and 'terrestrial plant | leaf particle |' in comp2.category: # to cover cases where comp1 category is air and comp2 category is terrestrial plant | leaf particle | something
            df_app2=df_alg_mat.loc[(df_alg_mat['sendingcompartmentcategory']=="abiotic | air | air - default") &(df_alg_mat['receivingcompartmentcategory']=="terrestrial plant | leaf particle") & (df_alg_mat['enabled']=='True')]
            df_app=df_app.append(df_app2) 
        if 'abiotic | air | air - default' in comp2.category and 'terrestrial plant | leaf |' in comp1.category: # to cover cases where comp2 category is air and comp1 category is terrestrial plant | leaf | something
            df_app2=df_alg_mat.loc[(df_alg_mat['receivingcompartmentcategory']=="abiotic | air | air - default") &(df_alg_mat['sendingcompartmentcategory']=="terrestrial plant | leaf") & (df_alg_mat['enabled']=='True')]
            df_app=df_app.append(df_app2) 
        if 'abiotic | air | air - default' in comp2.category and 'terrestrial plant | leaf particle |' in comp1.category: # to cover cases where comp2 category is air and comp1 category is terrestrial plant | leaf particle | something
            df_app2=df_alg_mat.loc[(df_alg_mat['receivingcompartmentcategory']=="abiotic | air | air - default") &(df_alg_mat['sendingcompartmentcategory']=="terrestrial plant | leaf particle") & (df_alg_mat['enabled']=='True')]
            df_app=df_app.append(df_app2) 
...
def create_trans_mat(inputs,dict_inputs): # function to create transition matrix
    ...
    # tm=np.empty((mat_dim,mat_dim), dtype=object) # create empty n x n matrix with flexible objects (could be numbers or arrays
    tm_c=np.zeros((mat_dim,mat_dim), dtype=object) # create empty n x n matrix with flexible objects (could be numbers or arrays) to hold constant transfer factors only
    tm_v={} # to hold variable transfer factors in a dictionary
    ...
    for chem_index, chem in enumerate(chem_list_clean): # loop over chemicals
        ...
        comp_objects_dict=define_comp.define_comp(currentchemical,met_dict) # create dictionary of compartment objects    
            ...
            for col_index, comp_col in enumerate (comp_list): # loop over columns (sending compartments)
                ...
                if link: 
                    for alg in app_algs: # loop over applicable algorithms
                        ...
                        if (type(transfer_factor))==np.ndarray:
                            transfer_factor=np.nan_to_num(transfer_factor)
                        else:
                            if np.isnan(transfer_factor):
                                transfer_factor=0
                        ...
                        if (type(transfer_factor)==float or type(transfer_factor)==np.float64 or type(transfer_factor)==np.float32) and alg_instance.doestransformchemical=='False': # if tf is a float (not error) and algorithms does not involved transformation
                            tm_c[int(chem_index*ncomp+row_index)][int(chem_index*ncomp+row_index)]=tm_c[int(chem_index*ncomp+row_index)][int(chem_index*ncomp+row_index)]-transfer_factor # sending compartment tf is negative
                            tm_c[int(chem_index*ncomp+col_index)][int(chem_index*ncomp+row_index)]=tm_c[int(chem_index*ncomp+col_index)][int(chem_index*ncomp+row_index)]+transfer_factor # receiving compartment tf is positive                        

                        if (type(transfer_factor)==np.ndarray) and alg_instance.doestransformchemical=='False': # if tf is a float (not error) and algorithms does not involved transformation
                            key_s=(int(chem_index*ncomp+row_index),int(chem_index*ncomp+row_index)) # key for sending compartment
                            key_r=(int(chem_index*ncomp+col_index),int(chem_index*ncomp+row_index)) # key for receiving compartment
                            if key_s in tm_v.keys():
                                tm_v[key_s]=tm_v[key_s]-transfer_factor # sending compartment tf is negative
                            else:
                                tm_v[key_s]=-transfer_factor
                            if key_r in tm_v.keys():    
                                tm_v[key_r]=tm_v[key_r]+transfer_factor # receiving compartment tf is positive                        
                            else:
                                tm_v[key_r]=transfer_factor # receiving compartment tf is positive                        
                                

                        if row_index==col_index and (type(transfer_factor)==float or type(transfer_factor)==np.float64 or type(transfer_factor)==np.float32 or type(transfer_factor)==np.ndarray) and alg_instance.doestransformchemical=='True': # if tf is a float (not error) and algorithms does involve transformation
    ...
    # tm[tm==None]=0    # convert None to zeros    
    df_tm=pd.DataFrame(tm_c,index=ind_name,columns=ind_name)
    ...
    return(tm_c,tm_v,sm,vmu,df_tm,df_sm,df_vmu,comp_objects_dict)

## for qa only
     

# row_index=0# air in source 
# col_index=2 # air in n1
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 

# row_index=2# air in n1 
# col_index=18 # surf soil in n1 
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 
...
# for k in df_tm_v.keys(): 
#     if k[1]==0: # replace value with sending compartment index 
#         print (k)
#         print (nanmax(df_tm_v[k]))

# row_index=2# air in n1 
# col_index=84 # surf soil in n1 
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 
...
# row_index=0# air in n1 
# col_index=14 # surf soil in n1 
# comp_row=comp_list[row_index]
# comp_col=comp_list[col_index]
# comp1_name=comp_row
# comp2_name=comp_col 
