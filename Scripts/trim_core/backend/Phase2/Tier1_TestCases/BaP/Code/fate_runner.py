...
import time
import pandas as pd
import convert_lc,mlib,vol_elem,comp,prop,dep_rates,solve_ode,gen_conc # import supporting modules to process inputs and auto generate code to define objects
...
inputs['simulation_name']='tnc0'
inputs['master_library_file']=r'icf_master_library_03212016_propertyexporter.txt'
# inputs['pseudo_library_file']=r'TNC0_1_pseudo_library_objects.txt'
inputs['met_file']=r'MetData_streamlined.csv'
inputs['allowexchange_file']=r'AllowExchange.csv'
inputs['litterfall_file']=r'LitterFallRate.csv'
inputs['ve_file']=r'TNC0 (2) Volume Elements.txt'
# inputs['pseudo_ve_file']=r'TNC0_2_pseudo_volume_elements.txt'
inputs['comp_file']=r'TNC0 (3) Compartments.txt'
inputs['prop_file']=r'TNC0 (4) Properties.txt'
inputs['prop_file_2']=r'TNC0 (5) Other Properties.txt'
#inputs['prop_file_3']=r'.txt'
# inputs['plink_file']=r'TNC0_3_pseudo_link_properties.txt'
# inputs['dep_rates_file']=r'PS_DepRates_Properties.txt'
inputs['dep_rates_file']=r'TNC0 (1) Sources.txt'

inputs['simulation_chemicals']=['benzo(a)pyrene']
...
def read_inputs_write_classes(inputs): # function to to read inputs and auto generate code to define objects    
    ...
    df_points,df_parcels,df_ve=vol_elem.define_volume_elements(inputs) # reads volume element inputs into these DataFrames and write define_ve.py and define_pve.py  
    df_props,df_props_2,df_links=prop.define_properties(inputs) # read properties files into these DataFrames and writes define_scenario.py and define_attributes_props.py
    ...
    # df_comp,comp_dict=comp.define_compartments(inputs,df_parcels,df_ve,df_props,df_props_2,df_dr) # reads compartments inputs into DataFrame and writes define_comp.py
    df_comp,comp_dict=comp.define_compartments(inputs,df_parcels,df_ve,df_props,df_props_2) # reads compartments inputs into DataFrame and writes define_comp.py
    df_dr=dep_rates.define_deposition_rates(inputs,df_points,df_parcels,df_comp) # reads deposition rates into DataFrame and writes define_attributes_dep_rates.py

    dict_inputs={'met_dict':inputs['met_dict'],'df_lib':df_lib,'df_alg':df_alg,'df_alg_mat':df_alg_mat,'df_chem':df_chem,'df_points':df_points,'df_parcels':df_parcels,'df_ve':df_ve,'df_comp':df_comp,'comp_dict':comp_dict,'df_props':df_props,'df_links':df_links,'df_dr':df_dr}
    return(dict_inputs)
...
if __name__=='__main__':
    ...
    tm_start=time.time()
    tm_c,tm_v,sm,vmu,df_tm_c,df_sm,df_vmu,comp_objects_dict=create_trans_mat.create_trans_mat(inputs,dict_inputs) # call function to generate transition matrix, sources, volume-mass-concentration_units matrix   
    df_tm_v=pd.DataFrame(tm_v)
    print ('starting diff equation solving')
    ode_start=time.time()
    nt,df_nt=solve_ode.ode_sim(tm_c,tm_v,df_tm_c,sm,df_sm)
    ode_end=time.time()
    ...
    ofpn=os.path.join(path_output,'tm_constant.csv')
    df_tm_c.to_csv(ofpn,index=True)
    ofpn=os.path.join(path_output,'tm_variable.csv')
    df_tm_v.to_csv(ofpn,index=False)
    ...
    dft=df_nt.tail(1) 
    ofpn=os.path.join(path_output,'massanalysis.csv')
    dft.transpose().to_csv(ofpn,index=True)
    
# import numpy    
# a=[]
# for x in range(0,tm.shape[0]):
#     for y in range(0,tm.shape[0]):
#         a.append(type(tm[x][y]))
#         if type(tm[x][y])==numpy.ndarray:
#             print ("Yes")
# b=set(a)