...
path_legacy_inputs = os.path.abspath(os.path.join(path_code, f'trim_core{os.sep}backend{os.sep}', 'Legacy_Input_Files'))  # legacy inputs directory
path_inputs = os.path.abspath(path_code)  # lower case inputs directory
path_output = os.path.abspath(os.path.join(path_code, f'trim_core{os.sep}backend{os.sep}', 'output'))  # output directory

# key is sheetname, value is corresponding chemical name in pytrim
...
pytrim_fp = os.path.join(path_inputs, 'Result_Matrix_test.csv')  # path to pytrim results
...
for k, v in cross_walk.items():
    ...
    for hd in header:  # loop over old column names
        ...
        # convert to new trim convention
...
for var in pyvars[2:]:  # loop over py variables and find ratio of corresponding value in legacy file
    chem_name = None
    for k, v in cross_walk.items():  # find chem_name by looking within variable
        if v in var:
            chem_name = v
        # else:
        #     print(f'{10 * "*"} cannot find chem:{v} in {var}')
    var_name = var.split(chem_name + '_')[1]  # find var_name within variable
    # print(var)
    if var in leg_df[chem_name].columns:
        print(f'Comparing Legacy value and pyTRIM value for {var} ...')
...
ofp = \
    r'/Users/55284/Library/CloudStorage/OneDrive-ICF/Desktop/RTR_TRIM-FaTE/TRIM_builder_mac_2/TRIM_builder_new/Scripts'
...
ofpn = os.path.join(path_inputs, ofn)
